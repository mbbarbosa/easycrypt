...
lemma scaleRq2R2t_DM (p1 p2 : Rq) : 
  ...
  scaleRq2R2t (p1 + p2)%Rq = (scaleRq2R2t p1 + scaleRq2R2t p2)%R2t.
proof.
...
lemma upscaleRq_eqep_DM (p1 p2 : Rq) :
  upscaleRq (p1 + p2)%Rq (eq - ep) = (upscaleRq p1 (eq - ep) + upscaleRq p2 (eq - ep))%Rq.
proof.
rewrite -BigRq.XnD1CA.big_split /= polyXnD1_eqP => i rng_i; rewrite rcoeffZ_sum //= rcoeffD. 
have ->:
  (fun (i0 : int) => 
   (upscaleZq p1.[i0] (eq - ep) ** exp Rq.iX i0 + upscaleZq p2.[i0] (eq - ep) ** exp Rq.iX i0)%Rq)
  =
  (fun (i0 : int) => 
...
lemma scaleRp2R2t_DM (p1 p2 : Rp) : 
  ...
  scaleRp2R2t (p1 + p2)%Rp = (scaleRp2R2t p1 + scaleRp2R2t p2)%R2t.
proof.
...
lemma upscaleRq_eqep_MAl (p1 p2 : Rq) :
  upscaleRq (p1 * p2)%Rq (eq - ep) = (upscaleRq p1 (eq - ep) * p2)%Rq.
proof.
...
lemma scaleRp2Rq_MA (p1 p2 : Rp) : scaleRp2Rq (p1 * p2)%Rp = (scaleRp2Rq p1 * Rp2Rq p2)%Rq.
proof.
...
lemma scaleRq2R2_DM_invl (p1 : R2) (p2 : Rq) : 
  scaleRq2R2 (scaleR22Rq p1 + p2)%Rq = (p1 + scaleRq2R2 p2)%R2.
proof.
...
lemma eq_scaleRq2Rp_scaleroundRq2Rp (p : Rq) :
  scaleRq2Rp (p + h1)%Rq = scaleroundRq2Rp p.
proof.
...
lemma Rq2Rp_DG23 (b : Rq_vec) (s : Rq_vec) : 
       Rq2Rp (dotp b s + h1)%Rq = (dotp (Rqv2Rpv b) (Rqv2Rpv s) + Rq2Rp h1)%Rp.
...
lemma eq_comp23 (p : Rq) (m : R2) :
      Rp2Rppq ((scaleRq2Rp p) + (scaleR22Rp_var m (2 * ep - eq - 1)))%Rp
      =
      scaleRp2Rppq ((Rq2Rp p) + (scaleR22Rp m))%Rp.
proof.
...
lemma eq_comp_enc_altenc (b : Rp_vec) (s : Rq_vec) (m : R2) : 
  scaleRp2R2t ((dotp b (Rqv2Rpv s)) + Rq2Rp h1 + scaleR22Rp m)%Rp
  =
  scaleRq2R2t ((dotp (scaleRpv2Rqv b) s) + upscaleRq h1 (eq - ep) + scaleR22Rq m)%Rq.
proof.
...
lemma eq_comp_dec_altdec (b' : Rp_vec) (s : Rq_vec) (c : R2t):
  scaleRp2R2 ((dotp b' (Rqv2Rpv s)) + Rq2Rp h1 - scaleR2t2Rp c + Rq2Rp h2)%Rp
  =
  (scaleRq2R2 ((dotp (scaleRpv2Rqv b') s) + upscaleRq h1 (eq - ep) - scaleR2t2Rq c 
  + upscaleRq h2 (eq - ep)))%Rq.
proof.
have ->: 
  ((dotp b' (Rqv2Rpv s)) + Rq2Rp h1 - scaleR2t2Rp c + Rq2Rp h2)%Rp
  =
  ((dotp b' (Rqv2Rpv s)) + Rq2Rp h1 + Rq2Rp h2 - scaleR2t2Rp c)%Rp by ring.
have ->:
  ((dotp (scaleRpv2Rqv b') s + upscaleRq h1 (eq - ep) - scaleR2t2Rq c) + upscaleRq h2 (eq - ep))%Rq
  =
  ((dotp (scaleRpv2Rqv b') s + upscaleRq h1 (eq - ep) + upscaleRq h2 (eq - ep) - scaleR2t2Rq c))%Rq 
  by ring.
...
module Saber_PKE_Scheme : Scheme = {
   ...
   proc enc(pk: seed * Rp_vec, m: R2) : R2t * Rp_vec = {
      ...
      v' <- ((dotp b (Rqv2Rpv s')) + (Rq2Rp h1))%Rp;
      cm <- scaleRp2R2t ((v' + (scaleR22Rp m))%Rp);
   ...
   proc dec(sk: Rq_vec, c: R2t * Rp_vec) : R2 option = {
      ...
      b' <- c.`2;
      v <- ((dotp b' (Rqv2Rpv s)) + (Rq2Rp h1))%Rp;
      m' <- (scaleRp2R2 (v  - (scaleR2t2Rp cm) + (Rq2Rp h2)))%Rp;
...
module Saber_PKE_Scheme_Alt : Scheme = {
   ...
   proc enc(pk: seed * Rp_vec, m: R2) : R2t * Rp_vec = {
      ...
      b' <- scaleRqv2Rpv ((trmx _A) *^ s' + h);
      bq <- scaleRpv2Rqv b;
      v' <- ((dotp bq s') + (upscaleRq h1 (eq - ep)))%Rq;
      cm <- (scaleRq2R2t (v' + (scaleR22Rq m)))%Rq;
    ...
    proc dec(sk: Rq_vec, c: R2t * Rp_vec) : R2 option = {
      ...
      v <- ((dotp bq' s) + (upscaleRq h1 (eq - ep)))%Rq;
      m' <- (scaleRq2R2 (v - cmq + (upscaleRq h2 (eq - ep))))%Rq;
